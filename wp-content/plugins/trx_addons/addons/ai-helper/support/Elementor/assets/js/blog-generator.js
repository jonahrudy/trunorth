jQuery(document).ready(function(){'use strict';trx_addons_add_filter('trx_addons_filter_mediamanager_selector',function(selector){return selector+',.ai_helper_blog_generator_form_field_media:not(.inited)';});window.trx_addons_ai_helper_blog_generator=function(action,data,$button){trx_addons_msgbox_dialog('<div class="ai_helper_blog_generator_form_row">'
...
+'>'
+'</div>'
+'</div>',TRX_ADDONS_STORAGE['elm_ai_blog_generator_dialog_caption'],function(box){jQuery(document).trigger('action.init_hidden_elements',[box]);box.find('#ai_helper_blog_generator_set_content_images').on('change',function(e){box.find('.ai_helper_blog_generator_form_row_media').toggleClass('trx_addons_hidden',!jQuery(this).is(':checked'));});},function(btn,box){if(btn!==1){return;}
data.posts_total=box.find('#ai_helper_blog_generator_posts_total').val();if(data.posts_total<1){return;}
data.title_case=box.find('#ai_helper_blog_generator_title_case_title').val();data.cats_total=box.find('#ai_helper_blog_generator_cats_total').val();data.cats_per_post=box.find('#ai_helper_blog_generator_cats_per_post').val();data.tags_total=box.find('#ai_helper_blog_generator_tags_total').val();data.tags_per_post=box.find('#ai_helper_blog_generator_tags_per_post').val();data.comments_per_post=box.find('#ai_helper_blog_generator_comments_per_post').val();data.comments_every_post=box.find('#ai_helper_blog_generator_comments_every_post').val();data.set_featured_images=box.find('#ai_helper_blog_generator_set_featured_images').is(':checked')?1:0;data.set_content_images=box.find('#ai_helper_blog_generator_set_content_images').is(':checked')?1:0;data.content_image=box.find('#ai_helper_blog_generator_content_image').val();$button.addClass('trx_addons_loading');jQuery.post(TRX_ADDONS_STORAGE['ajax_url'],data).done(function(response){$button.removeClass('trx_addons_loading');var rez=trx_addons_parse_ajax_response(response);if(rez.error!==''){alert(rez.error);}else if(typeof rez.data=='undefined'||typeof rez.data.posts=='undefined'){alert(TRX_ADDONS_STORAGE['elm_ai_company_generator_bad_data']);}else{if(rez.data.posts.length==0){alert(TRX_ADDONS_STORAGE['elm_ai_blog_generator_no_posts']);}else{alert(TRX_ADDONS_STORAGE['elm_ai_blog_generator_posts_inserted'].replace('%d',rez.data.posts.length));}}});});};});